---
permalink: /
title: "About me"
excerpt: "About me"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---


<div class="notice--project" markdown="1">
I am a Computer Science Ph.D. candidate at [Rochester Institute of Technology](https://www.rit.edu/), advised by [Dr. Rui Li](https://ruililuci.com/). My research focuses on continual learning, parameter-efficient adaptation, and continual user modeling. Previously, I was a Research Scientist Intern at [Zillow Group](https://www.zillow.com/) and a Machine Learning Engineer at [Fusemachines](https://fusemachines.com/).

<p><a href="/files/CV.pdf" class="btn" download>Download CV</a></p>

### Research Interests
* **Machine Learning**: Continual Learning and Generalized Category Discovery, Recommendation Engine and Continual User Modeling
* **Computer Vision**: Object Segmentation, Re-identification, Tracking, Action Recognition
* **Bioinformatics**: Gene-Disease Association, Protein-Protein Interaction Prediction
</div>

## Research Highlights

<div class="research-grid" markdown="0">

<div class="research-card">
  <span class="research-card__venue">International Conference on Machine Learning · 2024</span>
  <h3 class="research-card__title">Bayesian Adaptation of Network Depth and Width for Continual Learning</h3>
  <p class="research-card__desc">A Bayesian continual-learning framework that dynamically adapts network depth and width to evolving tasks via beta-Bernoulli process priors.</p>
  <p class="research-card__links">
    <a href="https://openreview.net/pdf?id=c9HddKGiYk">Paper</a> ·
    <a href="https://github.com/jt4812/bayes_struc_adap_cl">Code</a> ·
    <a href="/publications/#thapa2024bayesian">Citation</a>
  </p>
</div>

<div class="research-card">
  <span class="research-card__venue">European Conference on Information Retrieval · 2026</span>
  <h3 class="research-card__title">Evolving Mixture of Low-Rank Experts for Continual User Modeling</h3>
  <p class="research-card__desc">A mixture-of-rank-1-experts architecture that enables parameter-efficient continual adaptation for sequential recommendation.</p>
  <p class="research-card__links">
    <a href="https://link.springer.com/chapter/10.1007/978-3-032-21300-6_21">Paper</a> ·
    <a href="https://github.com/jt4812/ecir26">Code</a> ·
    <a href="/publications/#thapa2026evolving">Citation</a>
  </p>
</div>

</div>

<p><a href="/publications/" class="btn btn--inverse">View All Publications</a></p>

<div class="notice--project" markdown="1">
### News
* (Jun 2026): Served as a reviewer for ECCV 2026 and NeurIPS 2026.
* (Apr 2026): Successfully defended my Ph.D. dissertation proposal.
* (Mar 2026): Our paper "[Evolving Mixture of Low-Rank Experts for Continual User Modeling](https://link.springer.com/chapter/10.1007/978-3-032-21300-6_21)" was accepted at ECIR 2026.
* (Sep 2025): Completed a summer internship as a Research Scientist Intern at Zillow Group.
* (Sep 2024): Served as a reviewer for ICLR 2025.
* (May 2024): Our paper "[Bayesian Adaptation of Network Depth and Width for Continual Learning](https://openreview.net/pdf?id=c9HddKGiYk)" was accepted at ICML 2024.
* (May 2023): Successfully defended and passed my Research Potential Assessment (RPA).
* (Aug 2022): Joined the Ph.D. program in Computing and Information Sciences at RIT as a Graduate Research Assistant.
* (Jan 2022): Promoted to Machine Learning Engineer III at Fusemachines.
* (Jan 2021): Taught the undergraduate course "Mathematics for AI," covering linear algebra, multivariate calculus, probability, statistics, and information theory.
* (Jan 2021): Promoted to Machine Learning Engineer II at Fusemachines.
* (Sep 2019): Joined Fusemachines as an Associate Machine Learning Engineer.
* (Sep 2019): Graduated with a bachelor's degree in Computer Engineering from Pulchowk Campus, Tribhuvan University, Nepal.
* (Jan 2019): Joined Leapfrog Technology as an AI Intern for a six-month internship.
* (Jan 2018): Shortlisted for the Fusemachines AI Fellowship and fuseAI Scholarship.
</div>
